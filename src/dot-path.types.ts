...
type PathJoin<TKey extends string, TItem, TRoot> = TItem extends Primitive
  ? TKey
  : TItem extends TRoot
    ? TKey
...
export type PathImpl<TItem, TRoot = TItem> = TItem extends readonly unknown[]
  ? number extends TItem["length"]
    ? PathJoin<NumberKey, TItem[number], TRoot>
    : RecordPath<TItem, TRoot, TupleKey<TItem>>
  : TItem extends object
    ? RecordPath<TItem, TRoot, RecordKey<TItem>>
    : never;
...
type ResolvePathPart<TItem, TPart extends string> =
  TPart extends RecordKey<NotNil<TItem>>
    ...
    : TPart extends NumberKey
      ? NotNil<TItem> extends readonly unknown[]
        ? NotNil<TItem>[number] | NilOf<TItem>
        : never
      : never;

type ResolvePathString<TItem, TPath extends string> = TPath extends `${infer THead}.${infer TTail}`
  ? ResolvePathString<ResolvePathPart<TItem, THead>, TTail>
  : ResolvePathPart<TItem, TPath>;
...
type Walk<TItem, TRoot, TExpectedType> = TItem extends readonly unknown[]
  ? number extends TItem["length"]
    ? WalkJoin<NumberKey, TItem[number], TRoot, TExpectedType>
    : WalkRecord<TItem, TRoot, TExpectedType, TupleKey<TItem>>
  : TItem extends object
    ? WalkRecord<TItem, TRoot, TExpectedType, RecordKey<TItem>>
    : never;
